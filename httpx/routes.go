...
func (s *Server) initRoutes() {
	...
	s.router.GET("/health/live", s.handleHealthCheck())
	s.router.GET("/health/ready", gin.WrapF(ghttp.HandleHealthJSON(s.healthcheck)))
	...
	if s.Config().Mode != "production" && s.Config().HTTP.EnableProfiler {
		...
		debugGroup := s.router.Group("/debug")
		debugGroup.GET("/vars", gin.WrapF(expvar.Handler().ServeHTTP))

		profilerGroup := debugGroup.Group("/pprof")
